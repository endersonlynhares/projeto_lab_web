...
<body>
                    ...
                    <ul class="nav col-12 col-lg-auto my-2 justify-content-center my-md-0 text-small">
                        ...
                        <li>
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#personal_infor">Informações Pessoais</a></li>
                            </ul>
                        </li>
                    </ul>
                </div>
    ...
    <div class="container p-3">
        <div class="row">
            <div class="col-4 card o-hidden text-white bg-primary m-3">
                <div class="card-body">
                    <i class="fa-solid fa-cash-register"></i>
                    <a href="cadastro_os.php" class="text-decoration-none text-white">Cadastrar Ordem de Servico</a>

                </div>
            </div>

            <div class="col-4 card o-hidden text-white bg-secondary m-3">
                <div class="card-body">
                    <i class="fa-solid fa-cash-register"></i>
                    <a href="gerar_relatorio.php" class="text-decoration-none text-white">Gerar Relatório</a>
                    ...
                    <tbody>
                        ...
                        if ($ordens_servico->num_rows) {
                            while ($os = $ordens_servico->fetch_object()) {
                                $cod_tec = (int) $os->cod_tecnico;
                                /*PEGANDO O TECNICO RESPONSÁVEL PELO OS*/
                                $sql = "SELECT tecnico.nome FROM tecnico WHERE tecnico.cod_tecnico = ?";
                                $stmt = $con->prepare($sql);
                                $stmt->bind_param('i', $cod_tec);
                                $stmt->execute();
                                $tecnico_os = $stmt->get_result()->fetch_assoc();
                                $tecnico_os = $tecnico_os['nome'];
                                /*PEGANDO O EQUIPAMENTO DA OS*/
                                $cod_equi = (int) $os->cod_equipamento;
                                $sql = "SELECT equipamento.equipamento FROM equipamento WHERE equipamento.id_equipamento = ?";
                                $stmt = $con->prepare($sql);
                                $stmt->bind_param('i', $cod_equi);
                                $stmt->execute();
                                $equip_os = $stmt->get_result()->fetch_assoc();
                                $equip_os = $equip_os['equipamento'];
                        ?>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $os->diagnostico; ?></td>
                                    <td><?php echo $os->observacoes; ?></td>
                                    <td>R$ <?php $valor = $os->orcamento;
                                            echo number_format($valor, 2)  ?></td>
                                    <td><?php echo $tecnico_os; ?></td>
                                    <td><?php echo $equip_os; ?></td>
                                    <td>
                                        <a href="#" class="btn btn-secondary btn-sm" onclick="editar(<?php echo $os->cod_OS ?>, '<?php echo $os->orcamento ?>')">Editar</a>
                                        <a href="#" class="btn btn-danger btn-sm" onclick="excluir(<?php echo $os->cod_OS ?>)">Excluir</a>
                                        <img style="display: none;" id="gif<?= $os->cod_OS ?>" class="px-2" src="imgs/wait.gif" width="40" alt="">
                                    </td>
                                </tr>
    ...
    <div class="modal fade" id="form_editar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Modo Edição</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="config/editar_os.php" method="POST">
                        <div>
                        ...
                        <div>
                            ...
                            <select class="form-control" name="status[]" id="status" required>
                                ...
                                <option value="Abandonado pelo cliente">Abandonado pelo cliente</option>
                        ...
                        <div class="">
                            <label for="orcamento_os">Orçamento: </label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text">R$</span>
                                </div>
                                <input type="text" class="form-control" name="orcamento_os" id="orcamento_os" onkeypress="$(this).mask('000.000.000.000.000.00', {reverse: true});" required>
                            </div>
                        </div>
                </div>
                <div class="modal-footer">
    ...
    <script src="bibliotecas/js/sb-admin.min.js"></script>
    <script src="bibliotecas/js/sb-admin-datatables.min.js"></script>
    <script src="bibliotecas/js/sb-admin-charts.min.js"></script>
    <script src="config/excluir.js"></script>
    <script src="config/editar.js"></script>
